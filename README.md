...
## Live programming from Emacs

Build the live host, then pass it the application source you want to run:

```sh
coil build tools/live.coil -o build/paper-live
./build/paper-live examples/minimal.coil
```

The host compiles the complete application through the `coil-experiments` live metaprogram, opens its native window on the macOS main thread, and writes its nREPL port to `.nrepl-port`. Keep that process running while you edit the same source file.

Load the Emacs client from the neighboring `coil-experiments` checkout:

```elisp
(add-to-list 'load-path
             "/Users/jimmyhmiller/Documents/Code/projects/coil-experiments/editor/emacs")
(require 'coil-live)
```

Open the application `.coil` file, run `M-x coil-live-connect`, and use:

| Key | Action |
| --- | --- |
| `C-c C-c` | Compile and atomically publish the top-level definition at point |
| `C-c C-r` | Publish the selected forms as one transaction |
| `C-c C-k` | Publish the complete buffer |
| `C-c C-z` | Show the retained live-runtime state |

The application source must define `main`, which starts its native event loop, and `redraw`, which requests a frame after a successful edit. `examples/minimal.coil` contains both. Function slots, dependency tracking, rollback, and schema migration are generated by the live metaprogram; the nREPL layer only transports source edits and diagnostics.
...
For felt, woven fabric, wood, cork, and more, use the [24 material presets](lib/paper/MATERIALS.md). For example, `(felt 0x798a6f)` or `(texture-angle (woven-linen 0xd1c2a6) 30.0)` works anywhere a stock does. Their fine surface relief responds to lighting independently of sheet elevation. Velvet, brushed aluminum, satin, glazed ceramic, polished leather, and varnished wood also have light-dependent finishes; the gallery can toggle those finishes for comparison. Run `coil run examples/materials.coil` to explore the interactive swatch library. The original paper presets retain their existing grain.
